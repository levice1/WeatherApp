...
private val responceForFragsDataModel: ResponceForFragsDataModel by activityViewModels()
    ...
    private fun parseMainWeatherData(responceData: JsonWeatherParse) {

        val forecastListCount = responceData.forecast.forecastday.count()
        ...
        //2day
        getImg(
            responceData.forecast.forecastday[1].day.condition.icon,
            binding.img2Day
        )
        if (forecastListCount > 3) {
            //3day
            getImg(
                responceData.forecast.forecastday[2].day.condition.icon,
                binding.img3Day
            )
            //4day
            getImg(
                responceData.forecast.forecastday[3].day.condition.icon,
                binding.img4Day
            )
            //5day
            getImg(
                responceData.forecast.forecastday[4].day.condition.icon,
                binding.img5Day
            )
            //6day
            getImg(
                responceData.forecast.forecastday[5].day.condition.icon,
                binding.img6Day
            )
        } else SendErrToTelegram().sendMessage("Weather App: forecastListCount > 3, update API KEY!")
    }
}